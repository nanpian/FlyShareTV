...
public class BasicNetwork implements Network {
	 ...
	 *            HTTP stack to be used
	 * @param bytePoolSize
	 *            Size of buffer pool that improves GC performance in copy
	 *            operations.
	 * @param defaultCharset
	 *            when Http Header doesn't offer the 'Content-Type:Charset', it
	 *            will be use.
	 */
	public BasicNetwork(HttpStack httpStack, int bytePoolSize,
	...
	public NetworkResponse performRequest(Request<?> request)
		...
		while (true) {
			...
			if (request.isCanceled()) {
				...
				throw new NetroidError(networkResponse);
			}
			...
			try {
				...
				responseContents = request.handleResponse(httpResponse,
						mDelivery);
