 ...
 * 
 */
package com.zgntech.core.exception;
	...
	private boolean handleException(Throwable ex) {
		...
		new Thread() {
			...
			public void run() {
				Looper.prepare();
				Log.e(TAG, "程序出现异常");
				//Toast.makeText(mContext, "很抱歉,程序出现异常,请返回上级界面或尝试", Toast.LENGTH_SHORT).show();
				Looper.loop();
			}
		}.start();
		//收集设备参数信息 
	...
	private String saveCrashInfo2File(Throwable ex) {
		...
		try {
			...
			if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
				String path = Environment.getExternalStorageDirectory().getAbsolutePath()+"/MatchBox/log/";
