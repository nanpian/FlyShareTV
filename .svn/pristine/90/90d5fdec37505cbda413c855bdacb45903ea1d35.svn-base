...
public class FlyShareTVdataHelper extends SQLiteOpenHelper {
    ...
    // 下载记录表
    public static final String TBL_DOWNLOAD_INFOS = "download_info";
    // 上下文
    ...
    final static String SQL_HISTORY = "create table if not exists "
            ...
            + "_uri char, _time char,_name char,_fav integer,_pos integer,down_fin integer,_thumb char)";
    ...
    final static String SQL_DOWNLOAD = "create table if not exists "
            + TBL_DOWNLOAD_INFOS
            + "(_id integer PRIMARY KEY AUTOINCREMENT, "  
                + "start_pos integer, end_pos integer, compelete_size integer,url char, movieId text, movieName text, downloadstatus INTEGER )";
