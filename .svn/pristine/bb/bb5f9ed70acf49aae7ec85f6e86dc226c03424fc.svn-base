...
import cn.fxdata.tv.activity.hotplay.HotplayClassifyActivity;
...
import cn.fxdata.tv.view.HomeSlideShow;
...
public class HotPlayFragment extends BaseTitleBarFragment {
    ...
    // 顶部的滚动广告条
    private HomeSlideShow mHomeSlideShow;
    ...
    private int[] images = new int[] { R.drawable.hot_image_01,
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View view = inflater.inflate(R.layout.frag_main_hotplay_video,
                container, false);
        // 顶部滚动广告条
        mHomeSlideShow = (HomeSlideShow) view.findViewById(R.id.home_slideshow);
        // 热播主界面
        scrollMainView = (ScrollView) view
                .findViewById(R.id.frag_hotplay_main_view);
        // 网络连接错误图片，支持点击
        ...
        // 网络连接时的转圈
        fxProgressBar = (FXProgressBar) view
                .findViewById(R.id.frag_hotplay_progressBar);
        ...
        searchEditText.setOnEditorActionListener(new OnEditorActionListener() {
            ...
            public boolean onEditorAction(TextView arg0, int actionId,
                ...
                if (actionId == EditorInfo.IME_ACTION_SEARCH) {
                    ...
                    bundle.putString(SearchResultActivity.SEARCH_KEY, searchEditText.getText().toString());
                    int schoolId = SettingsProvider.getIntCustomDefault(getActivity(), SettingsProvider.SETTINGS_HOT_PLAY_SCHOOL_ID, 2);
                    bundle.putInt(SearchResultActivity.SEARCH_SCHOOL_ID, schoolId);
    ...
    private void initClassify(View root) {
        ...
        initClassify(mVaiety,
                getResources().getString(R.string.fragment_1_variety),
                mListener, mVaietyAdapter);
        mDrama = (LinearLayout) root.findViewById(R.id.hotplay_drama);
        initClassify(mDrama, getResources()
                .getString(R.string.fragment_2_drama), mListener, mDramaAdapter);
        mMovie = (LinearLayout) root.findViewById(R.id.hotplay_movie);
        initClassify(mMovie, getResources()
                .getString(R.string.fragment_3_movie), mListener, mMovieAdapter);
        mAnime = (LinearLayout) root.findViewById(R.id.hotplay_anime);
        initClassify(mAnime, getResources()
                .getString(R.string.fragment_4_anime), mListener, mAnimeAdapter);
    }
    ...
    private void initClassify(LinearLayout root, String title,
            View.OnClickListener listener, BaseAdapter adapter) {
        TextView tvTitle = (TextView) root.findViewById(R.id.tv_item_name);
        tvTitle.setText(title);

        RelativeLayout rl = (RelativeLayout) root
                .findViewById(R.id.rl_hotplay_title);
        rl.setTag(new Integer(HotplayClassifyActivity.getIntFromString(title,
                getResources())));
        rl.setOnClickListener(listener);

        ImageView ivMore = (ImageView) root.findViewById(R.id.iv_item_more);
        ivMore.setTag(new Integer(HotplayClassifyActivity.getIntFromString(
                title, getResources())));
        ...
        if (requestCode == REQUEST_CODE_FOR_SEARCH_SCHOOL) {
            if (resultCode == Activity.RESULT_OK) {
                if (data != null) {
                    ...
                    String schoolName = bundle
                            .getString(SEARCH_SCHOOL_EXTRA_SCHOOL_NAME);
                    int schoolId = bundle
                            .getInt(SEARCH_SCHOOL_EXTRA_SCHOOL_SCHOOL_ID);
                    // shoolSelect.setText(school);
                    SettingsProvider.putString(getActivity(),
                            SettingsProvider.SETTINGS_HOT_PLAY_SCHOOL_NAME,
                            schoolName);
                    SettingsProvider.putInt(getActivity(),
                            SettingsProvider.SETTINGS_HOT_PLAY_SCHOOL_ID,
                            schoolId);
                    updateCurrentSchool();
                }
            }
    ...
    private void checkAndShowSchoolSearch() {
        ...
        boolean isFirstEnter = SettingsProvider.getBooleanCustomDefault(
                getActivity(), SettingsProvider.SETTINGS_FIRST_ENTER_HOT_PLAY,
                true);
        if (isFirstEnter) {
            SettingsProvider.putBoolean(getActivity(),
                    SettingsProvider.SETTINGS_FIRST_ENTER_HOT_PLAY, false);
            // 要启动学校搜索界面
            showActivityForResult(SearchSchoolActivity.class,
                    REQUEST_CODE_FOR_SEARCH_SCHOOL);
        }
        updateCurrentSchool();
    }
    ...
    private void updateCurrentSchool() {
        String school = SettingsProvider.getStringCustomDefault(getActivity(),
                SettingsProvider.SETTINGS_HOT_PLAY_SCHOOL_NAME, "null");
        if (!school.equals("null")) {
            ...
            loadData(schoolId);
            mHomeSlideShow.getBroadcastFromServer(schoolId);
        }
    }
