 ...
 *
 * The simple way to use this class is to call {@link BaseImageLoader#get(String, ImageListener)}
 * and to pass in the default image listener provided by
 * {@link BaseImageLoader#getImageListener(ImageView, int, int)}. Note that all function calls to
...
public abstract class BaseImageLoader {
     ...
     */
    public BaseImageLoader(RequestQueue queue, ImageCache imageCache) {
		mRequestQueue = queue;
		mCache = imageCache;
	}
    ...
    public static ImageListener getDesiredWidthImageListener(final ImageView imageView,
            final int defaultImageResId, final int errorImageResId , final int desiredWith) {
        return new ImageListener() {
            @Override
            public void onError(VolleyError error) {
                if (errorImageResId != 0) {
            ...
            public void onSuccess(ImageContainer response, boolean isImmediate) {
                if (response.getBitmap() != null) {
                    // Get the desired width of the bitmap.
                    Bitmap bitmap = response.getBitmap();
                    int width = bitmap.getWidth();
                    int height = bitmap.getHeight();
                    int desiredHeight = height * desiredWith / width;
                    imageView.setLayoutParams(new LayoutParams(desiredWith, (int) desiredHeight));
                    imageView.setImageBitmap(response.getBitmap());
                } else if (defaultImageResId != 0) {
                    imageView.setImageResource(defaultImageResId);
                }
     ...
     * If the default was returned, the {@link BaseImageLoader} will be invoked when the
