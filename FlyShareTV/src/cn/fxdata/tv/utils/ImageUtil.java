...
public class ImageUtil {
    ...
    public static final String SAVE_PHOTO_FLODER = Environment
            .getDataDirectory() + "/data/" + "FxApp/imagecache/";
     ...
     * 图片保存到SD卡的路径
     */
    public static final String SAVE_SD_PHOTO = Environment
            .getExternalStorageDirectory().getAbsolutePath() + "/FxApp/fx/";
    /**
     * 裁剪后的图片保存到SD卡的路径
     */
    public static final String SAVE_SD_PHOTO_TEMP = Environment
            .getExternalStorageDirectory().getAbsolutePath() + "/FxApp/temp/";
     ...
     * @return 图片对象
     */
    public Bitmap findBitmapInCacheAndLocal(Context mContext, int id,
            int width, boolean readCache) {
        Bitmap bitmap = null;
     ...
     * @return 图片对象
     */
    public Bitmap findBitmapInCacheAndLocal(String url, int width,
            boolean readCache) {
        Bitmap bitmap = null;
     ...
     * @return 图片对象
     */
    public Bitmap findBitmapInCacheAndLocalForBrand(String path, int width,
            boolean readCache) {
        Bitmap bitmap = null;
     ...
     */
    private static Bitmap getBitmapByWidthFromResource(Context context,
            int imgId, int width) {
        ...
        try {
            ...
            temBitmap = BitmapFactory.decodeResource(context.getResources(),
                    imgId, outOptions);
        } catch (Throwable t) {
            t.printStackTrace();
        }
    ...
    private static Bitmap getBitmapByWidth(String localImagePath, int width1) {
        ...
        try {
            ...
            Log.i("evening",
                    temBitmap.getHeight() + "=====" + temBitmap.getWidth());
     ...
     *            下载状�?的回调监�?
     */
    public synchronized Bitmap downloadImageByUrl(String imageUrl, int width)
        ...
        try {
            ...
            String storePath = FileUtil.getStorePath()
                    + ImageCache.getLogoKey(imageUrl);
     ...
     *            下载状�?的回调监�?
     */
    public synchronized Bitmap downloadImageByUrl(String imageUrl, String path,
            int width) throws OutOfMemoryError {
        if (TextUtils.isEmpty(imageUrl)) {
    ...
    private static Bitmap getBitmapByWidth(Bitmap bt, int newWidth) {
        ...
        // 缩放图片动作
        matrix.postScale(scaleWidth, scaleWidth);
        Bitmap bitmap = Bitmap.createBitmap(bt, 0, 0, (int) width,
    ...
    public static Bitmap getBitmapForSceen(String path) {
        ...
        try {
            ...
            if (outOptions.outHeight <= maxHeight
                    && outOptions.outWidth <= maxWidth) {
                outOptions.inJustDecodeBounds = false;
                outOptions.inPurgeable = true;
                outOptions.inInputShareable = true;
            ...
            // 当图片的宽或者高不小于最大�?时，则需要对图片处理以显示最佳效果�?
                ...
                // 根据宽高情况分别设置缩放比例
                if ((float) outOptions.outHeight / outOptions.outWidth > (float) maxHeight
                        / maxWidth) {
                    // 以高为标准缩�?
                    height = maxHeight;
     ...
     * @return bitmap 指定宽高的缩略图
     */
    public static Bitmap getThumbnailFromLocalImage(String localImagePath,
        ...
        try {
            ...
            if (temBitmap != null && temBitmap.getWidth() != width
                    && temBitmap.getHeight() != height) {
                Bitmap desBitmap = Bitmap.createScaledBitmap(temBitmap, width,
                        height, true);
                temBitmap.recycle();
                temBitmap = null;
                return desBitmap;
    ...
    public String createBitmapAddLogo(Bitmap src, Context mContext) {
        ...
        String string = "/sdcard/Tyloo/downloadimage/"
     ...
     * @return
     */
    public BitmapDrawable writStrToImg(Context context, Bitmap imgBit,
            String str) {
        ...
        Bitmap icon = Bitmap.createBitmap(newWidth, hight,
                Bitmap.Config.ARGB_8888); // 建立�?��空的BItMap
